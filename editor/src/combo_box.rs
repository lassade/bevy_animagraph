use bevy_egui::egui::{epaint::Shape, style::WidgetVisuals, *};
...
/// egui::ComboBox::from_id_source("enum_popup")
...
impl ComboBox {
    ...
    pub fn show_ui(self, ui: &mut Ui, menu_contents: impl FnOnce(&mut Ui)) -> Response {
        ...
        ui.horizontal(|ui| {
            ...
            let response = combo_box(ui, button_id, selected_text, menu_contents);
...
fn combo_box(
    ...
    crate::popup::popup_below_widget(ui, popup_id, &button_response, menu_contents);
...
fn button_frame(
    ...
    // `is_pointer_button_down_on` is private so we can't do:
    // response.is_pointer_button_down_on |= button_active;
    // let visuals = ui.style().interact(&response);

    // the alternative way is a bit verbose, maybe also ugly but it works
    let style = ui.style();
    let visuals = if !response.sense.interactive() {
        &style.visuals.widgets.noninteractive
    } else if response.is_pointer_button_down_on() || response.has_focus() || button_active {
    ...
    // TODO: Find a better alternative than `allocate_rect`
    //ui.advance_cursor_after_rect(outer_rect);
    // is the alternative to the above, but not ideal I guess
